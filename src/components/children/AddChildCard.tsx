...
/**
 * Composant qui affiche une carte "Ajouter un compte"
 * Affiche un design distinctif avec une icône + et des textes explicatifs
 */
const AddChildCard: React.FC<AddChildCardProps> = ({ onClick, className = '' }) => {
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <p className="mt-2 text-sm font-medium text-gray-900">{t('children.addChild')}</p>
        <p className="mt-1 text-xs text-gray-500">{t('children.createChild')}</p>
      </div>
    </div>
  );
};
